...
`
	...
	cmd.Flags().StringSliceVar(&c.flagConversionOpts, "conversion", []string{"format"}, "List of conversion opts. Allowed values are: [none, format, virtio]")
...
func (c *cmdMigrate) runInteractive(server lxd.InstanceServer) (cmdMigrateData, error) {
	...
	// Migration will move the image to the server and import is as
	...
	if !server.HasExtension("instance_import_conversion") {
		fmt.Println("Server does not support image conversion. Using normal migration")
...
func (c *cmdMigrate) run(cmd *cobra.Command, args []string) error {
	...
	supportedConversionOptions := []string{"none", "format"}
