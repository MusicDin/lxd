...
`
	...
	cmd.Flags().BoolVar(&c.flagNonInteractive, "non-interactive", false, "Prevent further interaction if migration form is incomplete and error out instead"+"``")
...
func (c *cmdMigrate) askServer() (lxd.InstanceServer, string, error) {
	...
	if c.flagToken != "" {
		...
		if c.flagNonInteractive {
			return nil, "", fmt.Errorf("Authentication token is required and cannot be empty")
		}
...
func (c *cmdMigrate) configure(server lxd.InstanceServer) (cmdMigrateData, error) {
	...
	if c.flagStorage != "" {
		...
		if len(storagePools) == 0 {
			return cmdMigrateData{}, fmt.Errorf("No storage pools available")
		}

		if !shared.ValueInSlice(c.flagStorage, storagePools) {
			return cmdMigrateData{}, fmt.Errorf("Storage pool %q not found", c.flagStorage)
		}
	...
	if config.InstanceArgs.Type == api.InstanceTypeContainer {
		...
		if len(c.flagMountPaths) > 0 {
			for _, path := range c.flagMountPaths {
				if !shared.PathExists(path) {
					return cmdMigrateData{}, fmt.Errorf("Invalid mount path %q: Path does not exist", path)
				}
	...
	} else if len(c.flagMountPaths) > 0 {
