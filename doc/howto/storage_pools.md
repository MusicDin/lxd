...
Create a storage pool named `pool1`:
    ...
    lxc storage create pool1 pure pure.gateway=https://<purestorage-address> pure.api.token=<purestorage-api-token> pure.iscsi.address=<iscsi-ip>

Create a storage pool named `pool2` that uses a PureStorage gateway with a certificate that is not trusted:

    lxc storage create pool2 pure pure.gateway=https://<purestorage-address> pure.gateway.verify=false pure.api.token=<purestorage-api-token> pure.iscsi.address=<iscsi-ip>

Create a storage pool named `pool3` that explicitly uses iSCSI to connect to PureStorage array:

    lxc storage create pool3 pure pure.gateway=https://<purestorage-address> pure.api.token=<purestorage-api-token> pure.mode=iscsi pure.iscsi.address=<iscsi-ip>
...
:input: lxc storage create my-remote-pool3 pure pure.gateway=https://<purestorage-address> pure.api.token=<purestorage-api-token> pure.iscsi.address=<iscsi-ip>
