...
for instType in container vm; do
    ...
    lxc exec "${inst}" -- curl -f -s --unix-socket /dev/lxd/sock -X GET lxd/1.0/storage-pools/${pool}/volumes | jq 'length == 0'
    ...
    ! lxc exec "${inst}" -- curl -f -s --unix-socket /dev/lxd/sock -X POST lxd/1.0/storage-pools/${pool}/volumes -H "Content-Type: application/json" -d '{"name": "vol-unknown-type"}' || false
    ! lxc exec "${inst}" -- curl -f -s --unix-socket /dev/lxd/sock -X POST lxd/1.0/storage-pools/${pool}/volumes -H "Content-Type: application/json" -d '{"name": "vol-type-not-allowed", "type": "container"}' || false
    ...
    lxc exec "${inst}" -- curl -f --unix-socket /dev/lxd/sock \
        -X POST lxd/1.0/storage-pools/${pool}/volumes \
        -H "Content-Type: application/json" \
        -d '{"name": "vol-01", "config": {"size": "10MiB"}, "type": "custom"}'

    lxc exec "${inst}" -- curl -f --unix-socket /dev/lxd/sock \
        -X POST lxd/1.0/storage-pools/${pool}/volumes/custom \
        -H "Content-Type: application/json" \
        -d '{"name": "vol-02", "config": {"size": "10MiB"}}'
    ...
    lxc exec "${inst}" -- curl -f -s --unix-socket /dev/lxd/sock -X GET lxd/1.0/storage-pools/${pool}/volumes | jq 'length == 2'
