...
EOF
    ...
    lxc exec "${inst}" -- curl -f --unix-socket /dev/lxd/sock -X POST "lxd/1.0/storage-pools/${pool}/volumes/custom/vol-02/snapshots" \
        -H "Content-Type: application/json" \
        -d '{"name": "snap-01"}'
    ...
    lxc exec "${inst}" -- curl -f -s --unix-socket /dev/lxd/sock -X DELETE "lxd/1.0/storage-pools/${pool}/volumes/custom/vol-02/snapshots/snap-01"
    ...
    lxc exec "${inst}" -- curl -f -s --unix-socket /dev/lxd/sock -X DELETE "lxd/1.0/storage-pools/${pool}/volumes/custom/vol-02/snapshots/snap-02"
    lxc exec "${inst}" -- curl -f -s --unix-socket /dev/lxd/sock -X GET "lxd/1.0/storage-pools/${pool}/volumes/custom/vol-02/snapshots?recursion=1" | jq -e 'length == 0'
