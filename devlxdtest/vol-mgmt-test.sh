...
EOF
    ...
    lxc exec "${inst}" -- curl -f -s --unix-socket /dev/lxd/sock -X PATCH lxd/1.0/instances/${inst} -H "Content-Type: application/json" -d "${attachReq}" | jq -r .id
...
EOF
    ...
    lxc exec "${inst}" -- curl -f -s --unix-socket /dev/lxd/sock -X PATCH lxd/1.0/instances/${inst} -H "Content-Type: application/json" -d "${detachReq}"
