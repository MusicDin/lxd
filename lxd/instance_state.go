...
func doInstanceStatePut(ctx context.Context, s *state.State, inst instance.Instance, req api.InstanceStatePut, op *operations.Operation) error {
	...
	case instancetype.Stop:
		// Stopping the instance ends the QEMU process while a snapshot with a bitmap has overlays on its disks.
		// Refuse while such a snapshot is in progress.
		unlock, err := storagePools.LockInstanceNBD(s, inst)
		if err != nil {
			return err
		}
	...
	case instancetype.Restart:
		...
		// Refuse while a snapshot with a bitmap is in progress or an NBD export writes any of its block volumes.
		unlock, err := storagePools.LockInstanceNBD(s, inst)
		if err != nil {
			return err
		}
