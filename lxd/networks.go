...
func networkPut(d *Daemon, r *http.Request) response.Response {
	...
	run := func(ctx context.Context, op *operations.Operation) error {
		err := doNetworkUpdateInner(n, req, targetNode, clientType, httpMethod, clustered)
		if err != nil {
			return err
		}
...
// doNetworkUpdateInner loads the current local network config, merges with the requested network config, validates
// and applies the changes. Will also notify other cluster nodes of non-node specific config if needed.
func doNetworkUpdateInner(n network.Network, req api.NetworkPut, targetNode string, clientType request.ClientType, httpMethod string, clustered bool) error {
	if req.Config == nil {
		req.Config = map[string]string{}
	}
