...
func autoUpdateImagesTask(d *Daemon) (task.Func, task.Schedule) {
	f := func(ctx context.Context) {
		...
		op, err := operations.OperationCreate(nil, s, "", operations.OperationClassTask, operationtype.ImagesUpdate, nil, nil, opRun, nil, nil)
		if err != nil {
...
func distributeImage(ctx context.Context, s *state.State, nodes []string, oldFingerprint string, newImage *api.Image) error {
	...
	for _, nodeAddress := range nodes {
		...
		client, err := cluster.Connect(nil, nodeAddress, s.Endpoints.NetworkCert(), s.ServerCert(), true)
		if err != nil {
...
func pruneExpiredImagesTask(d *Daemon) (task.Func, task.Schedule) {
	f := func(ctx context.Context) {
		...
		op, err := operations.OperationCreate(nil, s, "", operations.OperationClassTask, operationtype.ImagesExpire, nil, nil, opRun, nil, nil)
		if err != nil {
...
func pruneLeftoverImages(s *state.State) {
	...
	op, err := operations.OperationCreate(nil, s, "", operations.OperationClassTask, operationtype.ImagesPruneLeftover, nil, nil, opRun, nil, nil)
	if err != nil {
...
func imageExport(reqContext context.Context, s *state.State, fingerprint string, projectName string, secret string) ([]response.FileResponseEntry, error) {
	...
	if address != "" {
		...
		_, err := cluster.Connect(reqContext, address, s.Endpoints.NetworkCert(), s.ServerCert(), false)
		if err != nil {
			return nil, err
		}
...
func autoSyncImagesTask(d *Daemon) (task.Func, task.Schedule) {
	f := func(ctx context.Context) {
		...
		op, err := operations.OperationCreate(nil, s, "", operations.OperationClassTask, operationtype.ImagesSynchronize, nil, nil, opRun, nil, nil)
		if err != nil {
