...
func storagePoolVolumeSnapshotsTypePostHandler(d *Daemon, r *http.Request) response.Response {
	...
	op, err := storagePoolVolumeSnapshotsTypePost(r.Context(), s, req, reqProjectName)
	if err != nil {
...
func storagePoolVolumeSnapshotsTypePost(reqContext context.Context, s *state.State, req api.StorageVolumeSnapshotsPost, reqProjectName string) (*operations.Operation, error) {
...
func storagePoolVolumeSnapshotsTypeGetHandler(d *Daemon, r *http.Request) response.Response {
	...
	if util.IsRecursionRequest(r) {
		snapshots, err := storagePoolVolumeSnapshotsTypeGet(r.Context(), s, projectName)
		if err != nil {
			return response.SmartError(err)
		}
	...
	urls, err := storagePoolVolumeSnapshotsTypeURLGet(r.Context(), s, projectName)
	if err != nil {
		return response.SmartError(err)
	}
...
func storagePoolVolumeSnapshotsTypeGet(reqContext context.Context, s *state.State, projectName string) ([]*api.StorageVolumeSnapshot, error) {
	details, err := request.GetCtxValue[storageVolumeDetails](reqContext, ctxStorageVolumeDetails)
	if err != nil {
		return nil, err
	}
...
func storagePoolVolumeSnapshotsTypeURLGet(reqContext context.Context, s *state.State, projectName string) ([]string, error) {
	details, err := request.GetCtxValue[storageVolumeDetails](reqContext, ctxStorageVolumeDetails)
	if err != nil {
		return nil, err
	}
...
func storagePoolVolumeSnapshotTypePostHandler(d *Daemon, r *http.Request) response.Response {
	...
	op, err := storagePoolVolumeSnapshotTypePost(r.Context(), s, snapshotName, req, reqProjectName)
	if err != nil {
		return response.SmartError(err)
	}
...
func storagePoolVolumeSnapshotTypePost(reqContext context.Context, s *state.State, snapshotName string, req api.StorageVolumeSnapshotPost, reqProjectName string) (*operations.Operation, error) {
	details, err := request.GetCtxValue[storageVolumeDetails](reqContext, ctxStorageVolumeDetails)
	if err != nil {
		return nil, err
	}
...
func storagePoolVolumeSnapshotTypePutHandler(d *Daemon, r *http.Request) response.Response {
	...
	err = storagePoolVolumeSnapshotTypePut(r.Context(), s, snapshotName, req, etag)
	if err != nil {
		return response.SmartError(err)
	}
...
func storagePoolVolumeSnapshotTypePut(reqContext context.Context, s *state.State, snapshotName string, req api.StorageVolumeSnapshotPut, reqETag string) error {
	details, err := request.GetCtxValue[storageVolumeDetails](reqContext, ctxStorageVolumeDetails)
	if err != nil {
		return err
	}
...
func storagePoolVolumeSnapshotTypePatchHandler(d *Daemon, r *http.Request) response.Response {
	...
	err = storagePoolVolumeSnapshotTypePatch(r.Context(), s, snapshotName, req, etag)
	if err != nil {
		return response.SmartError(err)
	}
...
func storagePoolVolumeSnapshotTypePatch(reqContext context.Context, s *state.State, snapshotName string, req api.StorageVolumeSnapshotPut, reqEtag string) error {
	details, err := request.GetCtxValue[storageVolumeDetails](reqContext, ctxStorageVolumeDetails)
	if err != nil {
		return err
	}
...
func pruneExpiredAndAutoCreateCustomVolumeSnapshotsTask(d *Daemon) (task.Func, task.Schedule) {
	f := func(ctx context.Context) {
		...
		if len(expiredSnapshots) > 0 {
			...
			op, err := operations.OperationCreate(nil, s, "", operations.OperationClassTask, operationtype.CustomVolumeSnapshotsExpire, nil, nil, opRun, nil, nil)
			if err != nil {
		...
		if len(volumes) > 0 {
			...
			op, err := operations.OperationCreate(nil, s, "", operations.OperationClassTask, operationtype.VolumeSnapshotCreate, nil, nil, opRun, nil, nil)
			if err != nil {
