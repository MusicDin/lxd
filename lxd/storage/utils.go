...
// nbdInstanceLockName returns the NBD lock name of an instance. An NBD session of the root volume of the instance
// holds the lock for its duration, and a start, a stop and a restart of the instance take it.
...
// nbdVolumeLockName returns the NBD lock name of a custom volume, which an NBD session of the volume holds for its
// duration. On a local pool the same volume name is a different volume on each member, so memberName scopes the
// lock to one member. It is empty for a remote pool, whose volume is one shared entity across the cluster.
...
// LockInstanceNBD acquires the NBD locks that guard an instance's root volume and each attached custom block
// volume against conflicting operations, such as starting or stopping the instance during an NBD export of one of
// the volumes, or stopping it during a snapshot with a bitmap. ISO and shared volumes are skipped, as NBD refuses them.
//
// Note that these are the same locks the NBD sessions and the snapshots with a bitmap hold, so an ongoing one makes
// it return a conflict error naming the operation that runs it, and holding them keeps a new one from starting
// until the returned cleanup function runs.
func LockInstanceNBD(s *state.State, inst instance.Instance) (func(), error) {
	...
	for _, devConf := range inst.ExpandedDevices() {
		...
		if devConf["source.type"] == cluster.StoragePoolVolumeTypeNameVM {
			...
			// GetVolumeNBD refuses shared volumes, so leave them unlocked and let instances that share
		...
		// GetVolumeNBD refuses ISO and shared volumes, so leave them unlocked and let instances that share
		// such a volume start at the same time.
