...
func (c *connectorSCSIFC) WaitDiskDevicePath(ctx context.Context, diskPathFilter block.DevicePathFilterFunc) (string, error) {
	logger.Warn("Waiting for SCSI/FC disk device path")
	_, ok := ctx.Deadline()
	if !ok {
	...
	if disks, err := os.ReadDir(block.DevDiskByID); err == nil {
		var scsiDevs []string
		for _, d := range disks {
			if strings.HasPrefix(d.Name(), scsiDiskDevicePrefix) {
	...
	go func() {
		...
		for {
			...
			case <-timer.C:
				...
				disks, err := os.ReadDir(block.DevDiskByID)
				if err == nil {
					var allDevs []string
					for _, d := range disks {
				...
				sysDevs, err := os.ReadDir("/sys/class/scsi_device")
				if err == nil {
					var devInfos []string
...
func (c *connectorSCSIFC) RemoveDiskDevice(ctx context.Context, devicePath string) error {
	logger.Warn("Removing SCSI/FC disk device", logger.Ctx{"device_path": devicePath})
	defer logger.Warn("SCSI/FC disk device removed", logger.Ctx{"device_path": devicePath})
...
func (c *connectorSCSIFC) WaitDiskDeviceResize(ctx context.Context, diskPath string, newSizeBytes int64) error {
	logger.Warn("Waiting for SCSI/FC disk device resize", logger.Ctx{"disk_path": diskPath, "new_size_bytes": newSizeBytes})
	_, ok := ctx.Deadline()
	if !ok {
		var cancel context.CancelFunc
		ctx, cancel = context.WithTimeout(ctx, 30*time.Second)
