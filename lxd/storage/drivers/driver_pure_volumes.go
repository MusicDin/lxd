...
func (d *pure) defaultVMBlockFilesystemSize() string {
	return pureDefaultVolumeSize
}
...
func (d *pure) UnmountVolume(vol Volume, keepBlockDev bool, op *operations.Operation) (bool, error) {
	...
	if vol.contentType == ContentTypeFS && filesystem.IsMountPoint(mountPath) {
		...
		d.logger.Debug("Unmounted PureStorage volume", logger.Ctx{"volName": vol.name, "path": mountPath, "keepBlockDev": keepBlockDev})
