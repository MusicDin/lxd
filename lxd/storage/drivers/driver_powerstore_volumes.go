...
func (d *powerstore) unmapVolume(vol Volume) error {
	...
	// device to disappear before returning. For NVMe the host-side device is removed
	// asynchronously after the array detaches the volume, so [Connector.WaitDiskDeviceGone] is
	// the only sync point. Using the deviceID returned by [connectors.RemoveDiskDevice] allows
	// safe detection of /dev/dm-X or NVMe path reuse by a different device.
