...
func (p *pureClient) getTarget() (*pureTarget, error) {
	...
	case pureModeNVMe:
		portal = p.driver.config["pure.nvme.address"]
	}
...
func (d *pure) connect() (revert.Hook, error) {
	...
	case pureModeNVMe:
		...
		nvmeAddr := d.config["pure.nvme.address"]
		hostNQN := d.hostNQN()

		serverUUID := d.state.ServerUUID
		_, stderr, err := shared.RunCommandSplit(d.state.ShutdownCtx, nil, nil, "nvme", "connect", "--transport", "tcp", "--traddr", nvmeAddr, "--nqn", targetNQN, "--hostnqn", hostNQN, "--hostid", serverUUID)
		if err != nil {
			return nil, fmt.Errorf("Failed to connect to PureStorage array %q via NVMe: %s\n%w", nvmeAddr, stderr, err)
		}
	}
