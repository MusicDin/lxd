...
func (p *pureClient) request(method string, url url.URL, reqBody io.Reader, reqHeaders map[string]string, respBody any, respHeaders map[string]string) error {
	...
	// Set request URL schema and host.
	url.Scheme = urlParts[0]
	url.Host = urlParts[1]
...
func (p *pureClient) createStoragePool(poolName string, size int64) error {
	...
	// Get default protection group names of the new storage pool.
...
func (p *pureClient) getVolume(poolName string, volName string) (*pureVolume, error) {
	...
	url := api.NewURL().Path("volumes").WithQuery("names", fmt.Sprintf("%s::%s", poolName, volName))
	err := p.requestAuthenticated(http.MethodGet, url.URL, nil, &resp)
	if err != nil {
		if isPureErrorNotFound(err) {
...
func (p *pureClient) createVolume(poolName string, volName string, sizeBytes int64) error {
	...
	url := api.NewURL().Path("volumes")
	url = url.WithQuery("names", fmt.Sprintf("%s::%s", poolName, volName))
...
func (p *pureClient) resizeVolume(poolName string, volName string, sizeBytes int64, truncate bool) error {
	...
	url := api.NewURL().Path("volumes")
	url = url.WithQuery("names", fmt.Sprintf("%s::%s", poolName, volName))
	url = url.WithQuery("truncate", fmt.Sprint(truncate))
...
func (p *pureClient) copyVolume(srcPoolName string, srcVolName string, dstPoolName string, dstVolName string, overwrite bool) error {
	...
	url := api.NewURL().Path("volumes")
	url = url.WithQuery("names", fmt.Sprintf("%s::%s", dstPoolName, dstVolName))
	url = url.WithQuery("overwrite", fmt.Sprint(overwrite))
...
func (p *pureClient) getVolumeSnapshots(poolName string, volName string) ([]pureVolume, error) {
	...
	url := api.NewURL().Path("volume-snapshots").WithQuery("source_names", fmt.Sprintf("%s::%s", poolName, volName))
	err := p.requestAuthenticated(http.MethodGet, url.URL, nil, &resp)
	if err != nil {
		if isPureErrorNotFound(err) {
			return nil, api.StatusErrorf(http.StatusNotFound, "Volume %q not found", volName)
...
func (p *pureClient) getVolumeSnapshot(poolName string, volName string, snapshotName string) (*pureVolume, error) {
	...
	url := api.NewURL().Path("volume-snapshots").WithQuery("names", fmt.Sprintf("%s::%s.%s", poolName, volName, snapshotName))
	err := p.requestAuthenticated(http.MethodGet, url.URL, nil, &resp)
	if err != nil {
		if isPureErrorNotFound(err) {
...
func (p *pureClient) connectHostToVolume(poolName string, volName string, hostName string) (bool, error) {
	url := api.NewURL().Path("connections")
	url = url.WithQuery("host_names", hostName)
	url = url.WithQuery("volume_names", fmt.Sprintf("%s::%s", poolName, volName))
