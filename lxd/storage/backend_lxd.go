...
// recvBlockVolumeFiller returns a function that receives the instance's volume.
// Function returns the volume size on success. Otherwise, it returns -1 for size and an error.
func (b *lxdBackend) recvBlockVolumeFiller(conn io.ReadWriteCloser, args migration.VolumeTargetArgs, op *operations.Operation) func(vol drivers.Volume, rootBlockPath string, allowUnsafeResize bool) (sizeInBytes int64, err error) {
	return func(vol drivers.Volume, rootBlockPath string, allowUnsafeResize bool) (int64, error) {
		...
		// if err != nil {
		// 	return -1, fmt.Errorf("Error getting VM block volume disk path: %w", err)
		// }
...
func (b *lxdBackend) imageConversionFiller(inst instance.Instance, conn io.ReadWriteCloser, args migration.VolumeTargetArgs, op *operations.Operation) func(vol drivers.Volume, rootBlockPath string, allowUnsafeResize bool) (sizeInBytes int64, err error) {
	return func(vol drivers.Volume, rootBlockPath string, allowUnsafeResize bool) (int64, error) {
		...
		to, err := os.OpenFile(imgPath, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0)
		if err != nil {
			return -1, fmt.Errorf("Error opening file for writing %q: %w", imgPath, err)
		}
		...
		if err != nil {
			...
			return -1, fmt.Errorf("qemu-img convert: failed to convert image from %q to %q format: %v", imgInfo.Format, "raw", err)
		}
...
func (b *lxdBackend) recvBlockVol(toFile *os.File, volName string, conn io.ReadWriteCloser, args migration.VolumeTargetArgs, op *operations.Operation) error {
	b.logger.Debug("Receive block volume started")
	defer b.logger.Debug("Receive block volume finished")
...
func (b *lxdBackend) CreateInstanceFromConversion(inst instance.Instance, conn io.ReadWriteCloser, args migration.VolumeTargetArgs, op *operations.Operation) error {
	...
	// If instance type is VM then receive only the block volume. Otherwise, receive the
	...
	if inst.Type() == instancetype.VM {
		...
		if slices.Contains(args.ConversionOptions, "format") {
			...
			volFiller.Fill = b.imageConversionFiller(inst, conn, args, op)
		} else {
			// If migration header supplies a volume size, then use that as block volume size instead of
			// pool default. This way if the volume being received is larger than the pool default size,
			// the block volume created will still be able to accommodate it.
			if args.VolumeSize > 0 {
				args.Config["size"] = fmt.Sprintf("%d", args.VolumeSize)
			}

			// If formatting is not required, receive the block volume directly into
			// the instance volume.
			volFiller.Fill = b.recvBlockVolumeFiller(conn, args, op)
		}
		...
		// Receive container filesystem directly into the instance volume.
