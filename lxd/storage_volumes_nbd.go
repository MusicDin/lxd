...
func storagePoolVolumeTypeNBDGet(d *Daemon, r *http.Request) response.Response {
	...
	conn, cleanup, err := storagePoolVolumeTypeNBDConnect(s, details, effectiveProjectName, false, args.Reuse)
	if err != nil {
		return response.SmartError(err)
	}
...
func storagePoolVolumeTypeNBDPost(d *Daemon, r *http.Request) response.Response {
	...
	conn, cleanup, err := storagePoolVolumeTypeNBDConnect(s, details, effectiveProjectName, true, false)
	if err != nil {
		return response.SmartError(err)
	}
