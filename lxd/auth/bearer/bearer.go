...
// it has not expired.
func Authenticate(token string, subject string, identityCache *identity.Cache, expectedIdentityType string) error {
	...
	// Validate that it is of the expected type. (When/if we have token authentication to the main API, we want to make
	// sure that DevLXD token bearers can't call the main API and vice versa).
	if entry.IdentityType != expectedIdentityType {
		return api.NewStatusError(http.StatusForbidden, "Token is not valid for this API")
	}

	// Verify the signature with the identities' secret.
