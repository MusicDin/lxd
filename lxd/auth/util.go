...
func IsTrusted(ctx context.Context) bool {
	reqInfo, _ := request.GetCtxInfo(ctx)
	return reqInfo.Trusted
}
...
func GetUsernameFromCtx(ctx context.Context) (string, error) {
	reqInfo, found := request.GetCtxInfo(ctx)
	if !found {
	...
	if reqInfo.Username != "" {
		return "", api.StatusErrorf(http.StatusInternalServerError, "Failed to get protocol from the request context")
	}
