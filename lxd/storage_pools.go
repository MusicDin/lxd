...
func storagePoolsPostHandler(d *Daemon, r *http.Request) response.Response {
	...
	lc, err := storagePoolsPost(r.Context(), s, req, targetNode, clientType, isClusterNotification(r))
	if err != nil {
		return response.SmartError(err)
	}
...
func storagePoolsPost(reqContext context.Context, s *state.State, req api.StoragePoolsPost, target string, clientType clusterRequest.ClientType, isClusterNotification bool) (*api.EventLifecycle, error) {
	// Quick checks.
	if req.Name == "" {
...
func storagePoolGetHandler(d *Daemon, r *http.Request) response.Response {
	...
	pool, etag, err := storagePoolGet(r.Context(), s, poolName, request.ProjectParam(r), withEntitlements, memberSpecific)
	if err != nil {
		return response.SmartError(err)
	}
...
func storagePoolGet(reqContext context.Context, s *state.State, poolName string, requestProjectName string, withEntitlements []auth.Entitlement, memberSpecific bool) (storagePool *api.StoragePool, etag any, err error) {
