...
// and the volume details from the request context. The request is forwarded when the volume or the instance using
// it lives on another cluster member. When the returned response is non-nil, the caller should return it
// immediately. operation names the caller's action in the precondition errors.
func storagePoolVolumeTypeBitmapInstance(s *state.State, r *http.Request, operation string) (inst instance.Instance, deviceName string, details storageVolumeDetails, resp response.Response) {
	...
	return inst, deviceName, details, nil
}
...
func storagePoolVolumeTypeBitmapsGet(d *Daemon, r *http.Request) response.Response {
	...
	inst, deviceName, details, resp := storagePoolVolumeTypeBitmapInstance(s, r, "Listing bitmaps")
	if resp != nil {
		return resp
	}
...
func storagePoolVolumeTypeBitmapsPost(d *Daemon, r *http.Request) response.Response {
	...
	inst, deviceName, _, resp := storagePoolVolumeTypeBitmapInstance(s, r, "Creating a bitmap")
	if resp != nil {
		return resp
	}
...
func storagePoolVolumeTypeBitmapGet(d *Daemon, r *http.Request) response.Response {
	...
	inst, deviceName, _, resp := storagePoolVolumeTypeBitmapInstance(s, r, "Getting a bitmap")
	if resp != nil {
		return resp
	}
...
func storagePoolVolumeTypeBitmapDelete(d *Daemon, r *http.Request) response.Response {
	...
	inst, deviceName, _, resp := storagePoolVolumeTypeBitmapInstance(s, r, "Deleting a bitmap")
	if resp != nil {
		return resp
	}
