...
// ActiveType returns the active version of a pending identity type.
// It returns an error for types that have no pending variant.
...
// PendingType returns the pending version of an active bearer identity type. A bearer identity is demoted to its
// pending type when it has no usable token, either before any token has been issued or after its token is revoked.
// It returns an error for types that have no pending variant (including the initial UI identity, which is exempt).
...
type BearerMetadata struct {
	...
	// It is zero when no token has been issued, or when the token has been revoked.
...
// A zero expiry clears the record, which is how token revocation is represented.
// It must be called in the same transaction as the corresponding signing key change to ensure
// the expiry is set only for active tokens.
func SetBearerIdentityTokenExpiry(ctx context.Context, tx *sql.Tx, identityID int64, expiry time.Time) error {
	// The expiry is signed into the token as a JWT NumericDate, which has second precision.
	// To ensure the expiry for a listed identity is identical to the one the token itself
	// carries, we truncate the expiry to second precision before storing it.
	if !expiry.IsZero() {
		expiry = expiry.UTC().Truncate(time.Second)
	}

	// Update token_expiry in place so any other metadata fields are preserved. The CASE seeds an empty
	// object for bearer identities created before token expiry was recorded, whose metadata is empty.
	// The bound value matches time.Time's JSON encoding, so it round-trips through BearerMetadata.
	q := `
UPDATE identities
SET metadata = json_set(
  CASE WHEN metadata = '' THEN '{}' ELSE metadata END,
  '$.token_expiry',
  ?
)
WHERE id = ? AND auth_method = ?`
	res, err := tx.ExecContext(ctx, q, expiry.Format(time.RFC3339Nano), identityID, AuthMethod(api.AuthenticationMethodBearer))
	if err != nil {
...
func (i *IdentitiesRow) ToAPI(idToGroups map[int64][]string, idToCertificates map[int64][]string) (*api.Identity, error) {
	...
	if i.AuthMethod == api.AuthenticationMethodTLS && !identityType.IsPending() {
		...
		// A zero expiry means no token has been issued, or the last issued token was revoked.
		if !metadata.TokenExpiry.IsZero() {
