...
findLeader:
	...
	client, err := cluster.Connect(nil, leaderInfo.Address, s.Endpoints.NetworkCert(), s.ServerCert(), true)
	if err != nil {
...
func internalClusterHeal(d *Daemon, r *http.Request) response.Response {
	migrateFunc := func(s *state.State, _ *http.Request, inst instance.Instance, targetMemberInfo *db.NodeInfo, live bool, startInstance bool, _ map[string]any, _ *operations.Operation) error {
		...
		dest, err := cluster.Connect(nil, targetMemberInfo.Address, s.Endpoints.NetworkCert(), s.ServerCert(), true)
		if err != nil {
			return err
		}
...
func autoHealClusterTask(d *Daemon) (task.Func, task.Schedule) {
	f := func(ctx context.Context) {
		...
		op, err := operations.OperationCreate(nil, s, "", operations.OperationClassTask, operationtype.ClusterHeal, nil, nil, opRun, nil, nil)
		if err != nil {
...
func autoHealCluster(_ context.Context, s *state.State, offlineMembers []db.NodeInfo) error {
	...
	dest, err := cluster.Connect(nil, s.LocalConfig.ClusterAddress(), s.Endpoints.NetworkCert(), s.ServerCert(), true)
	if err != nil {
		return err
	}
