...
func createFromMigration(s *state.State, r *http.Request, projectName string, profiles []api.Profile, req *api.InstancesPost) response.Response {
	...
	storagePool, args, resp := setupMigrationArgs(s, dbType, projectName, profiles, req)
	if resp != nil {
		return resp
	}
...
// setupMigrationArgs sets the database instance arguments and determines the storage pool to use.
func setupMigrationArgs(s *state.State, instType instancetype.Type, projectName string, profiles []api.Profile, req *api.InstancesPost) (storagePool string, instArgs *db.InstanceArgs, resp response.Response) {
