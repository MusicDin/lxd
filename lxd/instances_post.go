...
func createFromConversion(s *state.State, r *http.Request, projectName string, profiles []api.Profile, req *api.InstancesPost) response.Response {
	...
	// Currently, only VMs can be converted.
	if dbType != instancetype.VM {
		return response.BadRequest(fmt.Errorf("Instance type not supported %q", req.Type))
	}

	// Validate conversion options.
	convertOpts := req.Source.ConversionOptions
	for _, opt := range convertOpts {
		if opt == "none" {
			if len(convertOpts) > 1 {
				return response.BadRequest(fmt.Errorf("Conversion option %q cannot be used in combination with other options", "none"))
			}

			// Empty conversion options.
			convertOpts = []string{}
			break
		}
