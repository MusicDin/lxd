...
func createFromConversion(s *state.State, r *http.Request, projectName string, profiles []api.Profile, req *api.InstancesPost) response.Response {
	...
	if dbType != instancetype.VM {
		return response.BadRequest(fmt.Errorf("Only virtual machines can be created from image conversion. Instance type %q is not supported", req.Type))
	}

	// Validate conversion options.
	conversionOptions := req.Source.ConversionOptions
	for _, opt := range conversionOptions {
