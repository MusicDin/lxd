...
func instancePutHandler(d *Daemon, r *http.Request) response.Response {
	...
	resp, err := forwardedResponseIfInstanceIsRemote(s, r, projectName, instanceName, instanceType)
	if err != nil {
		return response.SmartError(err)
	}
	...
	op, err := instancePut(r.Context(), s, instanceName, projectName, req, etag)
	if err != nil {
		return response.SmartError(err)
	}
...
func instancePut(reqContext context.Context, s *state.State, instanceName string, projectName string, req api.InstancePut, reqETag string) (*operations.Operation, error) {
