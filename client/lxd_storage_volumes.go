...
func (r *ProtocolLXD) CreateStorageVolumeBitmap(pool string, volumeType string, volumeName string, bitmap api.StorageVolumeBitmapsPost) error {
	err := r.CheckExtension("storage_volume_block_tracking")
	if err != nil {
		return err
	}
	...
	_, _, err = r.query(http.MethodPost, path, bitmap, "")
	if err != nil {
		return err
	}
...
func (r *ProtocolLXD) DeleteStorageVolumeBitmap(pool string, volumeType string, volumeName string, bitmapName string) error {
	err := r.CheckExtension("storage_volume_block_tracking")
	if err != nil {
		return err
	}
