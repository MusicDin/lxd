...
func (r *ProtocolDevLXD) CreateStoragePoolVolumeSnapshot(poolName string, volType string, volName string, snapshot api.DevLXDStorageVolumeSnapshotsPost) error {
	url := api.NewURL().Path("storage-pools", poolName, "volumes", volType, volName, "snapshots").URL
	r.setURLQueryAttributes(&url)

	_, _, err := r.query(http.MethodPost, url.String(), snapshot, "")
	if err != nil {
		return err
	}
