...
func (r *ProtocolLXD) CreateInstanceBitmap(instanceName string, bitmap api.StorageVolumeBitmapsPost) error {
	...
	// Send the request
	_, _, err = r.query(http.MethodPost, path+"/"+url.PathEscape(instanceName)+"/bitmaps", bitmap, "")
	if err != nil {
		return err
	}
